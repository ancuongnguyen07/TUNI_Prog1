...
msg = """You␣will␣rejoice␣to␣hear␣that␣no␣disaster␣has␣accompanied␣the
...
while i + maxChar < n:
    ...
    if nextLastIndex < n and  msg[nextLastIndex] == '␣': 
        rowList.append(msg[i: nextLastIndex])
    ...
    # max length doesn't contain enough words, nextLast is not space
        ...
        for k in range(len(wordsInRow) - 1):
            ...
            if remainder > 0:
                wordsInRow[k] += '␣'
                remainder -= 1
        rowList.append('␣'.join(wordsInRow).rstrip())
        i = cursor + 1

rowList.append(msg[i:])
print('\n'.join(rowList).rstrip())
"""